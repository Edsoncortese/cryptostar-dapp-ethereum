...
contract('StarNotary', async (accs) => {
  ...
  it('lets user2 buy a star and decreases its balance in ether', async() => {
    ...
    let totalValue = Number(balanceOfUser2BeforeTransaction) - Number(balanceAfterUser2BuysStar);
  ...
  it("lets users add token name and symbol to a star", async () => {
    ...
    assert.equal(getTokenName, "Chico Star Token");
    assert.equal(getTokenSymbol, "CST");
  });
